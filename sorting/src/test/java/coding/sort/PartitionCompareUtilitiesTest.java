...
public class PartitionCompareUtilitiesTest {
  ...
  void testPartitionOfAnArray3WayDistinctElements(TestData arguments) {
    ...
    Integer[] input = arguments.getInput();
    // method under test
    int[] indexOfPartitionedPosition =
        PartitionUtilities.partitionDuplicateEntriesInArray(input, 0, input.length - 1);
  ...
  @ParameterizedTest(name = "Running permutation {index}")
  @ArgumentsSources({@ArgumentsSource(SortInputDuplicatesProvider.class)})
