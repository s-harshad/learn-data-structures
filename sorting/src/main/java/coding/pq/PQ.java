 ...
 * This single class implements both MIN and MAX PQ.
 *
 * @param <KEY> generic
 */
final public class PQ<KEY extends Comparable<KEY>> {

    private KEY[] pq; // data-structure to hold the keys.
    private int numberOfElements; // current total
    final private Comparator<KEY> comparator; // determines MAX or MIN PQ
    ...
    /**
     * Remove the current min or max key from PQ.
     * Min or Max depends on the  {@code Comparator} used.
     *
     * @return removed key
     */
    public KEY delete() {
    ...
    /**
     * Retrieve and return the current min or max key from PQ.
     * Min or Max depends on the  {@code Comparator} used.
     *
